...
for file_name in files_to_process:
    ...
    ax1.set_title(f"Linear Fit - {material_name}", fontsize=16)
    ax1.set_ylabel("Integrated Intensity [AU]",fontsize=14)
    ax1.legend()
    ax1.grid(True, linestyle='--', alpha=0.6)

    ax2.scatter(x, res_lin, color='darkblue', s=25)
    ax2.axhline(0, color='red', linestyle='--')
    ax2.set_xlabel("Concentration [mM]",fontsize=14)
    ax2.set_ylabel("Residuals",fontsize=14)
    ax2.grid(True, linestyle='--', alpha=0.6)

    plt.tight_layout()
    # CHANGED TO .jpg
    plt.savefig(f"Linear_Fit_{material_name}.jpg")
    plt.close()
    ...
    ax3.set_title(f"Polynomial Fit (Degree {poly_degree}) - {material_name}", fontsize=16)
    ax3.set_ylabel("Integrated Intensity [AU]" ,fontsize=14)
    ax3.legend()
    ax3.grid(True, linestyle='--', alpha=0.6)

    ax4.scatter(x, res_poly, color='darkblue', s=25)
    ax4.axhline(0, color='red', linestyle='--')
    ax4.set_xlabel("Concentration [mM]", fontsize=14)
    ax4.set_ylabel("Residuals",fontsize=14)
    ax4.grid(True, linestyle='--', alpha=0.6)

    plt.tight_layout()
    # CHANGED TO .jpg
    plt.savefig(f"Polynomial_Fit_{material_name}.jpg")
    plt.close()
    ...
    print(f"JPEG Plots (100 DPI) saved for {material_name}.")